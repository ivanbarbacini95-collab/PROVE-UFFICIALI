...
function signalDashboardCard(id,numericValue){
  // v8.2.4 clean baseline: registra il value senza applicare effetti visivi.
...
async function explorerJson(path,timeout=12000){
  ...
  throw lastError||new Error('Explorer indexer unavailable');
}
...
async function loadRewardHistory({append=false,showFeedback=true}={}){
  ...
  try{
    ...
    try{ result=await fetchRewardHistoryFromExplorer({offset:startOffset,maxPages:append?10:50,onProgress:progress}); }
      ...
      console.warn('Explorer indexer unavailable, uso LCD',indexerError);
...
(()=>{
  ...
  function health(){let score=100,risks=[];if(!state.address){score=0;risks.push(['Wallet','Not loaded'])}if(state.validators.length>0&&state.validators.length<2){score-=12;risks.push(['Concentrazione','Single validator'])}if(num(state.weightedCommission)>.1){score-=12;risks.push(['Commission','Above 10%'])}if(num(state.apr)<4&&state.address){score-=8;risks.push(['APR','Below 4%'])}if(num(state.rewards)>num(state.staked)*.01){score-=6;risks.push(['Reward','Compounding recommended'])}return {score:Math.max(0,score),risks}}
  function renderHealth(){const h=health(),score=document.getElementById('proHealthScore'),ring=document.getElementById('scoreRingValue'),verdict=document.getElementById('scoreVerdict'),list=document.getElementById('riskList');if(score)score.textContent=h.score;if(ring)ring.style.strokeDashoffset=String(314*(1-h.score/100));if(verdict)verdict.textContent=h.score>=85?'Excellent':h.score>=70?'Solid':h.score>=50?'Needs improvement':'Critical';if(list)list.innerHTML=h.risks.length?h.risks.map(r=>`<div class="risk-item"><span>${r[0]}</span><strong>${r[1]}</strong></div>`).join(''):'<div class="risk-item"><span>Risks</span><strong>No issues detected</strong></div>'}
...
/* v8.4.0 — QUEUED STABLE PER-DIGIT ODOMETER
...
(() => {
  ...
  function animateStableNumber(id,formatted,numericValue){
    ...
    // l'ultimo value ricevuto e riproducilo appena termina il passaggio corrente.
