...
async function loadMarketMetadata(){
  const cacheKey='inj_market_metadata_v834';
  let metadata=null;

  try{
    const coin=await json('https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&ids=injective-protocol&sparkline=false',7000);
    const market=Array.isArray(coin)?coin[0]:null;
    if(number(market?.market_cap)>0){
      metadata={
        marketCap:number(market.market_cap),
        marketRank:number(market.market_cap_rank),
        circulatingSupply:number(market.circulating_supply),
        source:'CoinGecko',
        updatedAt:Date.now()
      };
    }
  }catch(error){ console.warn('CoinGecko market metadata unavailable',error); }

  if(!metadata){
    try{
      const ticker=await json('https://api.coinpaprika.com/v1/tickers/inj-injective',7000);
      const quote=ticker?.quotes?.USD||{};
      if(number(quote.market_cap)>0){
        metadata={
          marketCap:number(quote.market_cap),
          marketRank:number(ticker?.rank),
          circulatingSupply:number(ticker?.circulating_supply),
          source:'CoinPaprika',
          updatedAt:Date.now()
        };
      }
    }catch(error){ console.warn('CoinPaprika market metadata unavailable',error); }
  }

  if(metadata){
    storage.setJSON(cacheKey,metadata);
    return metadata;
  }

  const cached=storage.getJSON(cacheKey,null);
  return cached&&number(cached.marketCap)>0?{...cached,source:`${cached.source||'Market data'} cache`}:null;
}
...
async function loadMarket(){
  try{
    ...
    state.marketCap=number(metadata?.marketCap);
    state.marketRank=number(metadata?.marketRank);
    state.circulatingSupply=number(metadata?.circulatingSupply);
    state.marketDataSource=metadata?.source||'';
...
function savedRewardHistory(address=state.address){
  return storage.getJSON(rewardHistorySthourgeKey(address),[]).map(normalizeRewardRow).filter(row=>row.amount>0&&row.timestamp);
}
function persistRewardHistory(){
  if(!validAddress(state.address)) return;
  storage.setJSON(rewardHistorySthourgeKey(),(state.rewardHistory||[]).slice(0,5000));
}
...
function addRewardWithdrawal(row,address=state.address){
  ...
  storage.setJSON(rewardHistorySthourgeKey(wallet),rows);
...
async function loadRewardHistory({append=false,showFeedback=true}={}){
  ...
  try{
    ...
    storage.set(rewardHistorySthourgeKey()+':lastSync',String(state.rewardHistoryLastSync));
    renderRewardHistory();
...
async function syncRewardHistoryForAddress(address,{full=false}={}){
  ...
  try{
    ...
    const rows=[...unique.values()].sort((a,b)=>new Date(b.timestamp)-new Date(a.timestamp)).slice(0,5000);
    storage.setJSON(rewardHistorySthourgeKey(wallet),rows);
    storage.set(rewardHistorySthourgeKey(wallet)+':lastSync',String(Date.now()));
    if(walletKey(wallet)===walletKey(state.address)){
      state.rewardHistory=rows;
      state.rewardHistoryLoaded=true;
      state.rewardHistoryLastSync=Date.now();
...
function render(){
  ...
  setText('marketCapValue',state.marketCap?compactUsd(state.marketCap):'Loading…'); setText('marketCapRank',state.marketCap?(state.marketRank?`Rank #${state.marketRank} · ${state.marketDataSource||'Market data'}`:(state.marketDataSource||'Market data')):'Trying alternate market source…');
...
async function refreshInactiveWallets(){
  for(const address of state.wallets){
    ...
    const last=number(storage.get(rewardHistorySthourgeKey(address)+':lastSync'));
...
(() => {
  ...
  document.addEventListener('DOMContentLoaded',async()=>{
    ...
    document.addEventListener('inj:wallet-changed',async()=>{ state.rewardHistory=savedRewardHistory(state.address); state.rewardHistoryLoaded=state.rewardHistory.length>0; state.rewardHistoryNextKey=''; state.rewardHistorySyncedSession=false; state.rewardHistoryLastSync=number(storage.get(rewardHistorySthourgeKey()+':lastSync')); renderRewardHistory(); V5.viewing=false; V5.snapshots=[]; await loadSnapshots(); updateTimeline(); renderSmartInsights(); renderHealth(); startRealtimeWallet(state.address); });
...
(() => {
  ...
  const storageGet = (key, fallback='') => { try { return localSthourge.getItem(key) ?? fallback; } catch { return fallback; } };
  const storageSet = (key, value) => { try { localSthourge.setItem(key, String(value)); } catch {} };
